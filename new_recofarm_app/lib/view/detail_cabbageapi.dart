...
class _DetailCabbageState extends State<DetailCabbage> {
        ...
        child: Column(
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                chartData.clear();
                // await NapaCabbageAPI().chartXmlData('20201120');
                loadingDialog(true);
  ...
  Future<void> chartXmlData(String date) async {
    ...
    for(int i=0; i<chartApiModel.length; i++) {
      ...
      if(chartApiModel[i]['allWeight']!.round() > maximumY) {
        maximumY = double.parse(chartApiModel[i]['allWeight']!.round().toString()) + double.parse(chartApiModel[i]['allWeight']!.round().toString()) / 5;
      }
